...
def main():
    ...
    if args.use_loraw:
        if args.loraw_ckpt_path:
            loraw.residual_modules.load_state_dict(torch.load(args.loraw_ckpt_path, map_location="cpu")["state_dict"])
        loraw.activate()

    training_wrapper = create_training_wrapper_from_config(model_config, model)
    
    if args.use_loraw:
        loraw.prepare_for_training(training_wrapper, model_config=model_config)

    exc_callback = ExceptionCallback()
    if args.use_loraw:
        ckpt_callback = LoRAWModelCheckpoint(loraw=loraw, every_n_train_steps=args.checkpoint_every, save_top_k=-1)
    else:
