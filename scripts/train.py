...
from loraw.callbacks import LoRAWModelCheckpoint
...
def main():
    ...
    if args.use_loraw:
        loraw = create_loraw_from_config(model_config, model)

    if args.pretrained_ckpt_path:
        copy_state_dict(
            model,
            torch.load(args.pretrained_ckpt_path, map_location="cpu")["state_dict"],
        )

    if args.pretransform_ckpt_path:
        model.pretransform.load_state_dict(
            torch.load(args.pretransform_ckpt_path, map_location="cpu")["state_dict"]
        )

    if args.use_loraw:
        if args.loraw_ckpt_path:
            loraw.residual_modules.load_state_dict(
                torch.load(args.loraw_ckpt_path, map_location="cpu")["state_dict"]
            )
        loraw.activate()
    ...
    if args.use_loraw:
        ckpt_callback = LoRAWModelCheckpoint(loraw=loraw, every_n_train_steps=args.checkpoint_every, dirpath=checkpoint_dir, save_top_k=-1)
    else:
    ...
    if args.use_loraw:
        trainer.fit(
            training_wrapper, train_dl, ckpt_path=None
        )
    else:
        trainer.fit(
            training_wrapper, train_dl, ckpt_path=args.ckpt_path if args.ckpt_path else None
        )
